...
class ArrayToolsTest extends BuddySuite {
	public function new() {
		...
		describe("ArrayTools.find()", {
			...
			it("should be true when a matched value exists in the array", {
				[0, 1, 2]
				.find(function(x) return x == 1).should.be(true);
			});
		...
		describe("ArrayTools.zip()", {
			...
			it("should be success", {
				[1]
				.zip(["foo"]).same([new Tuple2(1, "foo")]).should.be(true);

				[1, 2, 3]
				.zip(["foo", "bar", "baz"]).same([new Tuple2(1, "foo"), new Tuple2(2, "bar"), new Tuple2(3, "baz")]).should.be(true);
			});
		});
		...
		describe("ArrayTools.zipStringMap()", {
			...
			it("should be success", {
				[1]
				.zipStringMap(["foo"]).same(["1" => "foo"]).should.be(true);

				[1, 2, 3]
				.zipStringMap(["foo", "bar", "baz"]).same(["1" => "foo", "2" => "bar", "3" => "baz"]).should.be(true);
			});
		});
		...
		describe("ArrayTools.head()", {
			...
			it("should take from any with filter", {
				[1, 2, 3]
				.head(function(x) return x == 2).should.be(2);
				[1, 2, 3]
				.head(function(x) return false).isEmpty().should.be(true);
			});
		});
		...
		describe("ArrayTools.flatten()", {
			...
			it("should pass when it is given [ [1, 2] ]", {
				[[1, 2]]
				.flatten().should.containExactly([1, 2]);
			});

			it("should pass when it is given [ [1, 2], [], [3, 4], [5, 6] ]", {
				[[1, 2], [], [3, 4], [5, 6]]
				.flatten().should.containExactly([1, 2, 3, 4, 5, 6]);
			});
		});
