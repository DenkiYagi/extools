...
class LangTools {
	...
	public static macro function combine(objects:Array<ExprOf<{}>>):Expr {
		if (objects.length <= 0)
			return Context.error("Not enough arguments", Context.currentPos());
		if (objects.length == 1)
			return objects[0];
	...
	public static function same(value1:Dynamic, value2:Dynamic):Bool {
		#if js
		if (eq(value1, value2) || NullTools.isNull(value1) && NullTools.isNull(value2))
			return true;
		#else
		if (eq(value1, value2))
			return true;
		...
		while (stack.length > 0 && loop++ < 20) {
			...
			#if js
			if (eq(a, b) || NullTools.isNull(a) && NullTools.isNull(b))
				continue;
					...
					switch (Type.typeof(b)) {
						case TClass(bCls) if (eq(aCls, bCls)):
							if (Std.is(a, Array)) {
								...
								if (neq(aDate.getTime(), bDate.getTime()))
									return false;
								continue;
							} else if (Std.is(a, Bytes)) {
								var aBytes:Bytes = cast a;
								var bBytes:Bytes = cast b;
								if (neq(aBytes.length, bBytes.length))
									return false;
								for (i in 0...aBytes.length) {
									if (neq(aBytes.get(i), bBytes.get(i)))
										return false;
								}
								continue;
								...
								if (neq(keys.length, [for (k in bMap.keys()) k].length))
									return false;
								for (key in keys) {
			...
			if (neq(keys.length, Reflect.fields(b).length))
				return false;
			for (k in ignoreKeys)
				keys.remove(k);
			for (key in keys) {
